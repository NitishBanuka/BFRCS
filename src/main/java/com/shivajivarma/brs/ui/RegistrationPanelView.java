...
public class RegistrationPanelView extends BaseView implements View{
	...
	public long getMobile() {
		String mobileText = mobile.getText().trim();  // Get the text from the mobile field
		if (mobileText.isEmpty()) {
			throw new IllegalArgumentException("Mobile number cannot be empty.");
		}
		try {
			return Long.parseLong(mobileText);  // Parse the text as a long
		} catch (NumberFormatException e) {
			throw new IllegalArgumentException("Invalid mobile number format. Please enter a valid number.");
		}
	}
